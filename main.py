...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user.id != 1 or not current_user.id:
            return abort(403)
    ...
    if register_form.validate_on_submit():
        ...
        if not user:
            new_user = User(
                ...
                password=werkzeug.security.generate_password_hash(register_form.password.data)
            )
            ...
            flash("An account has already been created with this email please login.")
